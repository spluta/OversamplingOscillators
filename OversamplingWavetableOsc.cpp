...
struct ShaperOS : public BufUnit {
    float mOffset;
    float mPrevIn;
    VariableOversampling<> oversample;
    float *osBuffer;
    int m_oversamplingIndex;
};
...
void ShaperOS_Ctor(ShaperOS* unit);
void ShaperOS_next_1(ShaperOS* unit, int inNumSamples);
void ShaperOS_next_k(ShaperOS* unit, int inNumSamples);
void ShaperOS_next_a(ShaperOS* unit, int inNumSamples);
...
void ShaperOS_Ctor(ShaperOS* unit) {
    ...
    Print("ShaperOS_Ctor %f\n", samplerate);

    VariableOversampling<> oversample;

    oversample.reset(samplerate);
    unit->m_oversamplingIndex = 1;//sc_clip((int)ZIN0(2), 0, 4);
    oversample.setOversamplingIndex(unit->m_oversamplingIndex);

    unit->osBuffer = oversample.getOSBuffer();

    unit->oversample = oversample;

    unit->mPrevIn = ZIN0(1);

    SETCALC(ShaperOS_next_a);
    ShaperOS_next_a(unit, 1);
...
float force_inline ShaperOSPerform(ShaperOS* unit, const float* table0, const float* table1, float in, float offset, float fmaxindex) {
    float out;
    //unit->oversample.upsample(in);

    for (int k = 0; k < unit->oversample.getOversamplingRatio(); k++){ 
        //Print("ShaperOSPerform %d %d %d\n", in, unit->oversample.getOversamplingRatio(), unit->m_oversamplingIndex);
        float findex = offset + in * offset;
        findex = sc_clip(findex, 0.f, fmaxindex);
        int32 index = (int32)findex;
        float pfrac = findex - (index - 1);
    ...
    // if (unit->m_oversamplingIndex != 0)
    //   out = unit->oversample.downsample();
    // else
    //   out = unit->osBuffer[0];
    return out;
    ...
    //     float val = val1 + val2 * pfrac;
    //     //unit->osBuffer[0] = val;
    //     //val = unit->oversample.downsample();
    //     return val;
}
...
void ShaperOS_next_a(ShaperOS* unit, int inNumSamples) {
    ...
    //const float oversampling_index = ZIN0(2);

    LOOP1(inNumSamples, float fin = ZXP(in); ZXP(out) = ShaperOSPerform(unit, table0, table1, fin, offset, fmaxindex););
...
//     if (fin == unit->mPrevIn) {
//         LOOP1(inNumSamples, ZXP(out) = ShaperOSPerform(table0, table1, fin, offset, fmaxindex););
//     } else {
//         float phaseinc = (fin - unit->mPrevIn) * offset;
//         unit->mPrevIn = fin;

//         LOOP1(inNumSamples, ZXP(out) = ShaperOSPerform(table0, table1, fin, offset, fmaxindex); fin += phaseinc;);
//     }
// }
